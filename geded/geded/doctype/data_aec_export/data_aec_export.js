...
frappe.ui.form.on("Data AEC Export", {
  before_save(frm) {
    if (frm.is_new()) {
      frappe.show_alert({
        message:__('Season Deleted successfully'),
        indicator:'green'
      }, 5);
    }
  },
  setup(frm) {
    ...
    frappe.realtime.on("data_import_refresh", ({ data_import }) => {
      ...
      frappe.model.clear_doc("Data AEC Export", frm.doc.name);
      frappe.model.with_doc("Data AEC Export", frm.doc.name).then(() => {
	frm.refresh();
      });
    });
	...
	refresh(frm) {
		...
		if (frm.doc.status.includes("Success")) {
          ...
          if (!frm.is_new()) {
            frm.set_df_property("season", "read_only", 1);
	...
	show_import_status(frm) {
		frappe.call({
			method: "geded.geded.doctype.data_aec_export.data_aec_export.get_import_status",
			args: {
	...
	export_errored_rows(frm) {
		open_url_post(
			"/api/method/geded.geded.doctype.data_aec_export.data_aec_export.download_errored_template",
			{
				data_import_name: frm.doc.name,
			}
		);
	...
	export_import_log(frm) {
		open_url_post(
			"/api/method/geded.geded.doctype.data_aec_export.data_aec_export.download_import_log",
			{
				data_import_name: frm.doc.name,
			}
		);
