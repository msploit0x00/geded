...
from geded.geded.doctype.data_aec_export.importer import Importer
...
class DataAECExport(Document):

    def before_insert(self):
        query = f"delete from `tabVolume Of Member Exports` where `tabVolume Of Member Exports`.season = {self.season}"
        frappe.db.sql(query)

    def after_insert(self):
        log = frappe.get_all("Data Import Log", {"data_import": self.name})
        for l in log:
            frappe.delete_doc("Data Import Log", l["name"])
            
    def validate(self):
        doc_before_save = self.get_doc_before_save()
        if (not (self.import_file or self.google_sheets_url)
                or (doc_before_save
                    and doc_before_save.import_file != self.import_file) or
            (doc_before_save
    ...
    def get_importer(self):
        return Importer("Volume Of Member Exports", data_import=self)
...
def get_preview_from_template(data_import,
                              import_file=None,
                              google_sheets_url=None):
    return frappe.get_doc("Data AEC Export",
                          data_import).get_preview_from_template(
                              import_file, google_sheets_url)


@frappe.whitelist()
def form_start_import(data_import):
    return frappe.get_doc("Data AEC Export", data_import).start_import()


def start_import(data_import):
    """This method runs in the background job"""
    data_import = frappe.get_doc("Data AEC Export", data_import)
    try:
        check_season_value(data_import)
...
def check_season_value(data_import):
    season_value = data_import.get("season")
    if not season_value:
        frappe.throw("Season field value is required for import")

    # query = f"delete from `tabVolume Of Member Exports` where `tabVolume Of Member Exports`.season = {season_value} AND `tabVolume Of Member Exports`.creation < '{str(get_datetime())}'"
    # frappe.db.sql(query)

    # records_to_delete = frappe.get_all(
    #     "Volume Of Member Exports",
    #     filters={"season": season_value, "creation": ["<", str(get_datetime())]})

    # print(records_to_delete)

    # for record_name in records_to_delete:
    #     frappe.delete_doc("Volume Of Member Exports", record_name["name"])
...
def download_errored_template(data_import_name):
    data_import = frappe.get_doc("Data AEC Export", data_import_name)
...
def get_import_status(data_import_name):
    ...
    total_payload_count = frappe.db.get_value("Data Import", data_import_name,
                                              "payload_count")
...
def import_file(doctype,
    ...
    data_import = frappe.new_doc("Data AEC Export")
